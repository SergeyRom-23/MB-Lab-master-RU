...
def is_writeable(filepath):
    try:
        ...
        logger.warning("Отказано в записи для %s", filepath)
    return False
...
def get_data_path():
    ...
    logger.info("Ищем ретаргетинг данных в папке %s...", simple_path(data_dir))

    if not os.path.isdir(data_dir):
        logger.critical("Данные об инструменте не найдены. Пожалуйста, проверьте ваш каталог аддонов Blender.")
        return None
...
def get_configuration():
    ...
    logger.critical("База данных конфигурации не найдена. Пожалуйста, проверьте ваш каталог аддонов Blender.")
    return None
...
def get_blendlibrary_path():
    ...
    logger.critical("Библиотека моделей не найдена. Пожалуйста, проверьте ваш каталог аддонов Blender.")
    return None
            ...
            if os.listdir(lib_path):
                for database_file in os.listdir(lib_path):
                    ...
                    if "json" in extension or "bvh" in extension:
                        ...
                        logger.warning("Неизвестное расширение файла в %s", simple_path(lib_path))

        else:
            logger.warning("путь к данным %s не найден", simple_path(lib_path))
    return result
...
def bounding_box(verts_coo, indices, roundness=4):
    ...
    for idx in indices:
        if len(verts_coo) > idx:
            ...
            logger.warning("Ошибка в расчете ограничительной рамки: индекс %s не в verts_coo (len(verts_coo) = %s)",
                           idx, len(verts_coo))
            return None
...
def correct_morph(base_form, current_form, morph_deltas, bboxes):
    ...
    for d_data in morph_deltas:
        ...
        if str(idx) in bboxes:
            ...
            logger.warning("Индекс %s не в базе данных", idx)
    logger.info("Морфинг исправлен в %s сек", time.time()-time1)
...
def check_version(m_vers, min_version=(1, 5, 0)):
    ...
    if len(mesh_version) < 5:
        logger.warning("Текущий гуманоид имеет неправильный формат для версии")
        return False
...
def looking_for_humanoid_obj():
    ...
    """
    logger.info("Ищем гуманоидного объекта ...")
    if bpy.app.version < (2, 80, 74):
        msg = "Извините, MB-Lab требуется Blender минимум версии 2.80.74 "
        logger.warning(msg)
        return("ERROR", msg)
...
def load_json_data(json_path, description=None):
    try:
        ...
        with open(json_path, "r") as j_file:
            ...
            if not description:
                logger.info("Json бд %s загружена за %s сек",
                            simple_path(json_path), time.time()-time1)
            else:
                logger.info("%s загружена из %s за %s сек",
                            description, simple_path(json_path), time.time()-time1)
            return j_database
    except IOError:
        if simple_path(json_path) != "":
            logger.warning("Не найден файл: %s", simple_path(json_path))
    except json.JSONDecodeError:
        logger.warning("Ошибка в json файле: %s", simple_path(json_path))
    return None
...
def disable_object_modifiers(obj, types_to_disable=[]):
    for modfr in obj.modifiers:
        ...
        if modifier_type in types_to_disable:
            ...
            logger.info("Изменения %s из %s может создать непредсказуемые результаты. MB-Lab отключил его",
...
def apply_modifier(obj, modifier):
    ...
    if modifier_name in obj.modifiers:
        ...
        try:
            ...
            logger.warning("Проблема в применении %s. Это модификатор выключен?", modifier_name)
...
def new_modifier(obj, name, modifier_type, parameters):
    if name in obj.modifiers:
        logger.info("Модификатор %s уже присутствует %s", modifier_type, obj.name)
    ...
    for parameter, value in parameters.items():
        if hasattr(_new_modifier, parameter):
            try:
                ...
                logger.info("Не удалось установить атрибуты '%s' модификатора %s", parameter, name)
...
def set_modifier_parameter(modifier, parameter, value):
    if hasattr(modifier, parameter):
        try:
            ...
            logger.info("Не удалось установить атрибуты '%s' модификатора %s", parameter, modifier)
...
def select_and_change_mode(obj, obj_mode):
    ...
    if obj:
        ...
        try:
            ...
            logger.debug("Выберите и измените режим %s = %s", obj.name, obj_mode)
        except AttributeError:
            logger.warning("Невозможно применить режим %s в %s", obj.name, obj_mode)
...
def set_object_visible(obj):
    if obj:
        logger.debug("Включите видимость %s ON/вкл", obj.name)
        ...
        for database_file in os.listdir(folderpath):
            ...
            if file_type in extension:
                if the_item not in items_list:
                    the_descr = "Загрузить и применить {0} из лаборатории".format(the_item)
...
def load_image(filepath):
    if os.path.isfile(filepath):
        logger.info("Загрузить изображение %s", os.path.basename(filepath))
        img = bpy.data.images.load(filepath, check_existing=True)
        img.reload()
    else:
        logger.info("Изображение %s не найдено", os.path.basename(filepath))
        ...
        if name in bpy.data.images:
            ...
            if bpy.data.images[name].source == "FILE":
                if os.path.basename(bpy.data.images[name].filepath) != name:
                    logger.warning("Название изображения %s из файла: %s",
                                   name, os.path.basename(bpy.data.images[name].filepath))
            return bpy.data.images[name]
        logger.warning("Изображение не получено, Изображение %s не найдено bpy.data.images", name)
        return None

    logger.warning("Изображение не получено. Название изображения %s", name)
    return None
...
def save_image(name, filepath, fileformat='PNG'):
    ...
    if img:
        ...
        logger.warning(
            "Изображение %s не может быть сохранено, так как его нет в bpy.data.images.", name)
...
def set_node_image(mat_node, mat_image):
    if mat_node:
        ...
        logger.warning("Назначение узла не выполнено. Изображение не найдено: %s", mat_image)
...
def get_material(material_name):
    for material in bpy.data.materials:
        if material.name == material_name:
            return material
    logger.warning("Материал %s не найден", material_name)
    return None
...
def get_material_nodes(material):
    ...
    logger.warning("Материал %s не имеет узлов", material.name)
    return None
...
def get_material_node(material_name, node_name):
    ...
    if not material_node:
        logger.warning("Узел не найден: %s в материале %s", node_name, material_name)
...
def get_node_output_value(node, index):
    if index < len(node.outputs):
        ...
        logger.warning("Socket [%s] не имеет default_value атрибутов", index)
        return None

    logger.warning("Socket [%s] не в диапазоне узла %s вывода", index, node.name)
    return None
...
def get_edit_bones(armature):
    if bpy.context.mode != 'EDIT_ARMATURE':
        logger.warning("Невозможно получить кости редактирования, потому что объект не находится в режиме редактирования.")
        return None
...
def get_pose_bones(armature):
    if bpy.context.mode != 'POSE':
        logger.warning("Невозможно получить кости позы, потому что объект не находится в режиме позы")
        return None
...
def set_bone_constraint_parameter(constraint, parameter, value):
    if hasattr(constraint, parameter):
        try:
            ...
            logger.info("Атрибуты не установлены '%s' ограничения %s",
...
def link_to_collection(obj):
    ...
    if obj.name not in bpy.data.objects:
        logger.error("Невозможно связать obj %s потому что это не в bpy.data.objects", obj.name)
        return
    ...
    if c is not None:
        if obj.name not in c.objects:
            ...
            logger.warning("Объект %s уже связан со сценой", obj.name)
    else:
...
def import_object_from_lib(lib_filepath, name, final_name=None, stop_import=True):
    if name != "":
        if stop_import:
            logger.info("Добавление объекта %s из %s", name, simple_path(lib_filepath))
            if name in bpy.data.objects:
                logger.warning("Объект %s уже в сцене. Импорт остановлен", name)
                return None

            if final_name:
                if final_name in bpy.data.objects:
                    logger.warning("Объект %s уже в сцене. Импорт остановлен", final_name)
                    return None

        append_object_from_library(lib_filepath, [name])
        obj = get_object_by_name(name)
        if obj:
            logger.info("Объект '%s' импортирован", name)
            if final_name:
                obj.name = final_name
                logger.info("Объект '%s' переименован '%s'", name, final_name)
            return obj

        logger.warning("Объект %s не найден в библиотеке %s", name, simple_path(lib_filepath))
    return None
...
def append_object_from_library(lib_filepath, obj_names, suffix=None):
    ...
    try:
        ...
        logger.critical("библиотека %s не найдена", lib_filepath)
