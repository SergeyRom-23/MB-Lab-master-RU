...
class HumanModifier:
    ...
    def __repr__(self):
        return "Изменить <{0}> с {1} свойством: {2}".format(
            self.name,
            len(self.properties),
            self.properties)
...
class HumanCategory:
    ...
    def get_all_properties(self):
        """
        Вернуть все свойства, относящиеся к категории,
        отсортировано и без двойных записей.
        """
        properties = []
        for modifier in self.modifiers:
    ...
    def __repr__(self):
        return "Категория {0} с {1} изменением".format(
            self.name,
            len(self.modifiers))
...
class Humanoid:
    ...
    def init_database(self,obj,character_identifier,rigging_type):
        ...
        logger.info("Определен гуманоид: {0}".format(character_identifier))
        ...
        logger.info("Загружено {0} категорий из базы данных morph".format(
            len(self.categories)))
    ...
    def init_character_data(self, morph_name):
        ...
        if components[0][:4] not in self.no_categories:
            if len(components) == 3:
                ...
                logger.warning("Не верное имя morph-а: {0}".format(morph_name))
    ...
    def reset_category(self, categ):
        ...
        logger.info("Категория сброшена в {0} сек".format(time.time()-time1))
    ...
    def automodelling(self,use_measures_from_GUI=False, use_measures_from_dict=None, use_measures_from_current_obj=False, mix=False):
        ...
        if self.morph_engine.measures_database_exist:
            ...
            logger.info("Персонаж описывается за {0} сек".format(time.time()-time2))
    ...
    def save_backup_character(self, filepath):
        ...
        new_filepath = os.path.join(dir_path,new_filename)
        logger.info("Сохранение резервной копии {0}".format(algorithms.simple_path(new_filepath)))
    ...
    def sync_obj_props_to_character_materials(self):
        ...
        for material_data_prop, value in self.character_material_properties.items():
            if hasattr(obj, material_data_prop):
                ...
                logger.warning("Материал {0}  не найден".format(material_data_prop))
        self.material_realtime_activated = True
    ...
    def correct_expressions(self, correct_all=False):
        ...
        logger.info("Выражение исправлено в {0} сек".format(time.time()-time1))
    ...
    def reset_character(self):
        ...
        logger.info("Персонаж сброшен в {0} сек".format(time.time()-time1))
        ...
        for meta_data_prop, value in self.character_metaproperties.items():
            if hasattr(obj, meta_data_prop):
                ...
                if "last" not in meta_data_prop:
                    logger.error("метаданные {0}.{1} не обнаружены".format(obj.name,meta_data_prop))
        self.metadata_realtime_activated = True


    def delete_all_properties(self):
        time1 = time.time() #TODO: usare obj.keys per lavorare solo sui valory applicati
        logger.info("Удаление пользовательских свойств")
        obj = self.get_object()
        ...
        for prop in props_to_delete:
            try:
                ...
                logger.info("Свойство {0} не было использовано этим персонажем".format(prop))

        armat = self.sk_engine.get_armature()
        logger.info("Свойство удалено за {0} сек".format(time.time()-time1))
    ...
    def generate_character(self,random_value,prv_face,prv_body,prv_mass,prv_tone,prv_height,prv_phenotype,set_tone_and_mass,body_mass,body_tone,prv_fantasy):
        logger.info("Генерация персонажа...")
    ...
    def calculate_transformation(self, tr_type):
        ...
        if transformation_id in self.transformations_data:
            ...
            logger.warning("{0} данные отсутствуют".format(transformation_id))
